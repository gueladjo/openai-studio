...
| `npm run clean` | Remove generated dependencies, build/package output, log files, and tool-generated Git refs under `.git/refs/codex` whose deep paths break Windows copies. Add `-- --dry-run` to preview the targets. |
...
Only one browser tab writes the workspace, coordinated through the Web Locks
API. The writer keeps ownership while attempting its final save on reload or
close. Autosaves and visibility checkpoints reduce the risk of lost changes, but
a browser shutdown cannot guarantee completion of an asynchronous save.
